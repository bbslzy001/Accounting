...
public class HomeRvItemDecoration extends RecyclerView.ItemDecoration implements RecyclerView.OnItemTouchListener
{
    private final HomeRvAdapter adapter;
    private final Context context;
    private HeaderCoordinate headerCoordinate;
    private HomeRvGroup currentGroup;
    private View currentHeaderView;

    public HomeRvItemDecoration(Context context, HomeRvAdapter adapter)
    {
    ...
    private void initHeaderCoordinate(RecyclerView recyclerView, int firstVisiblePosition)
    {
        ...
        if (firstVisibleViewType == HomeRvAdapter.HEADER_ITEM)  // 如果是列表头
        {
     ...
     * 创建一个新的悬浮列表头
     */
    private View createHeaderView(RecyclerView recyclerView, HomeRvGroup group)
    {
        View headerView = LayoutInflater.from(context).inflate(R.layout.rv_header_item_home, recyclerView, false);
